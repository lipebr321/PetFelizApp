...
namespace APIPetFeliz.DAO.Usuarios
{
    public class UsuariosDAO
    {
        ...
        public IEnumerable<UsuariosDTO> ListarPorId(int id)
        {
            var conexao = ConnectionFactory.Build();
            conexao.Open();
            ...
            try
            {
                ...
                return (IEnumerable<UsuariosDTO>)usuario;
        ...
        public void AlterarUsuario(UsuariosDTO usuario)
        {
            ...
            try
            {
                ...
                var sqlEstado = @"update Tb_Estado set Nome_Estado = @estado where Cod_Estado = @Id;";

                var idEstado = conexao.ExecuteScalarAsync<int>(sqlEstado, new
                {
                    Id = usuario.Estado.Cod_Estado,
                    estado = usuario.Estado.Nome_Estado
                });

                var sqlCidade = @"update Tb_Cidade set Nome_Cidade = @cidade where Cod_Cidade = @Id";

                var idCidade = conexao.ExecuteScalarAsync<int>(sqlCidade, new
                {
                    Id = usuario.Cidade.Cod_Cidade,
                    cidade = usuario.Cidade.Nome_Cidade,
                });
